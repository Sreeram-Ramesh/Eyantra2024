...
always @(posedge clk_3125) begin
    if (counter == 14) begin
        case (state)
            START:                                                  state = BIT0;
            BIT0:                  {state, rx_prev_msg} = {BIT1, rx, 7'b0000000};
            BIT1:   {state, rx_prev_msg} = {BIT2, rx_prev_msg[7], rx, 6'b000000};
            BIT2:  {state, rx_prev_msg} = {BIT3, rx_prev_msg[7:6], rx, 5'b00000};
            BIT3:   {state, rx_prev_msg} = {BIT4, rx_prev_msg[7:5], rx, 4'b0000};
            BIT4:    {state, rx_prev_msg} = {BIT5, rx_prev_msg[7:4], rx, 3'b000};
            BIT5:     {state, rx_prev_msg} = {BIT6, rx_prev_msg[7:3], rx, 2'b00};
            BIT6:      {state, rx_prev_msg} = {BIT7, rx_prev_msg[7:2], rx, 1'b0};
            BIT7:          {state, rx_prev_msg} = {PARITY, rx_prev_msg[7:1], rx};
            PARITY:                         {state, rx_prev_parity} = {STOP, rx};
            STOP:  begin
                {state, rx_parity, rx_complete} = {4'bzzzz, rx_prev_parity, 1'b1};
                if (rx_parity == rx_prev_msg[7] + rx_prev_msg[6] + rx_prev_msg[5] + rx_prev_msg[4] + rx_prev_msg[3] + rx_prev_msg[2] + rx_prev_msg[1] + rx_prev_msg[0]) rx_msg = rx_prev_msg;
